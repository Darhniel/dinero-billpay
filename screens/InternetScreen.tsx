import { StyleSheet, Text, TouchableOpacity, View, Image, SafeAreaView, ActivityIndicator, Modal } from 'react-native'
...
export default function InternetScreen({ navigation }: any) {
    const [isVisible, setIsVisible] = useState({ isError: false, isSuccess: false, responseMessage: "" });
    ...
    const { isError, responseMessage } = isVisible;



    async function userDetails() {
        try {
            const response = await fetch('https://api.dinerobillpay.com/api/v1/auth/bills/internet', {
                method: 'GET',
                headers: {
    ...
    function handleNetworkChange(network: string) {
        ...
        navigation.navigate('InternetDetails', { filtered });
    };
                        ...
                        data.map((item) => {
                            return (
                                <TouchableOpacity style={styles.item} onPress={() => { handleNetworkChange(item.network) }} key={item.network}>
                                    <Image source={{ uri: item.logo }} style={{ width: scale(40), height: scale(40) }} />
                ...
                <View style={{ flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#00000080" }}>
                    <View style={[styles.modal, { alignItems: "center", flex: 2 / 5, width: "95%" }]}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                userDetails();
                                setIsVisible({ isError: false, isSuccess: false, responseMessage: "" });
                            }}
                        >
